...
def main():
    ...
    # -------------------------------------------------------------------------
    # 1: Environment Handshake & Connection Isolation Engine [RESOLVED]
    ...
    try:
        ...
        # Connecting to a schema in Snowflake
        ...
        # Get a cursor or a SQL statement handle from the snowflake DBMS connection
        cs = ctx.cursor()
    ...
    # -------------------------------------------------------------------------
    # 2: Semi-Structured Polymorphic Schema Verification (DDL) [RESOLVED]
    # Execute a DDL statement to guarantee the target landing table exists.
    #
    # The table configuration MUST feature:
    #      - an active 'VARIANT' type data column to hold the raw unstructured
    #        polymorphic incoming document strings efficiently, and
    #      - a default transactional generation record ingestion timestamp.
    # -------------------------------------------------------------------------
    try:
        ### TODO 2 CODE START
        # Creating the table, with 2 colums as requested
        cs.execute("CREATE TABLE IF NOT EXISTS RAW_TRANSCRIPTS"
                   "(json_payload VARIANT, inserted_at TIMESTAMP_LTZ DEFAULT CURRENT_TIMESTAMP);")
    ...
    # -------------------------------------------------------------------------
    # 3: Safe Streaming Consumer Insertion Invariant [RESOLVED]
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            ### TODO 3 CODE START
            sql_statement = "INSERT INTO RAW_TRANSCRIPTS (JSON_PAYLOAD) SELECT PARSE_JSON(%s)"
            payload = json.dumps(json_data)
            # Bind values must be a sequence; trailing comma makes a 1-tuple,
